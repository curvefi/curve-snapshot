...
staking = "0x59f58431d4cBA2B7e9E8D78F064a8fa24C5134bF"
staking_contract = ERC20Contract(staking, provider=fantom_web3_provider)
...
lp_gauge_supply = lp_contract.balanceOf(staking, block_identifier=block)
...
ftm_per_lp, ftml_per_lp = balances[0] / lp_gauge_supply, balances[1] / lp_gauge_supply
...
total_sum = 0
for user in users:
    balance = staking_contract.balanceOf(user, block_identifier=block)
    if balance > 0:
...
with open(
    Path(BASE_DIR, "data", "ftml", "staking_snapshot.csv"), "w", newline=""
) as file:
    writer = csv.writer(file)
    writer.writerows(data)

with open(
    Path(BASE_DIR, "data", "ftml", "staking_overall.csv"), "w", newline=""
) as file:
    writer = csv.writer(file)
    writer.writerows(
        [
